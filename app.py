...
# ----------------------------------------------------------------------
# Multiple server endpoints (login & update)
# ----------------------------------------------------------------------
LOGIN_BASE_URLS = [
...
# Paths (assumed same on all servers)
LOGIN_PATH = "/MajorLogin"
UPDATE_PATH = "/UpdateSocialBasicInfo"
...
def perform_major_login(access_token, open_id):
    """
    Try all login servers until one returns a valid JWT token.
    Returns (token, used_login_url) or (None, None)
    """
    platforms = [8, 3, 4, 6]
            ...
            try:
                ...
                if response.status_code == 200:
                    ...
                    if data_dict and "token" in data_dict:
                        return data_dict["token"], base_url
            except Exception:
                continue
    return None, None
...
def perform_guest_login(uid, password):
    ...
    try:
        resp = requests.post("https://100067.connect.garena.com/oauth/guest/token/grant", data=payload, headers=headers, timeout=10, verify=False)
        data = resp.json()
...
def upload_bio_request(jwt_token, bio_text):
    """
    Try all update servers until one succeeds.
    Returns dict with status, and includes the used server URL.
    """
    for base_url in UPDATE_BASE_URLS:
        url = base_url + UPDATE_PATH
        try:
            data = BioData()
            data.field_2 = 17
    ...
    # All servers failed
    return {
        "status": "❌ All update servers failed",
        "code": 500,
        "bio": bio_text,
        "server_response": "N/A",
        "update_server_used": None
    }
...
def combined_bio_upload():
    ...
    if jwt_token:
        ...
        login_server_used = "Direct JWT (no login request)"
        ...
        if acc_token and login_openid:
            ...
            final_jwt, login_server_used = perform_major_login(final_access_token, final_open_id)
            ...
            if final_jwt:
                 ...
                 return jsonify({"status": "❌ JWT Generation Failed (all login servers tried)", "code": 500}), 500
        ...
        if final_open_id:
            final_jwt, login_server_used = perform_major_login(access_token, final_open_id)
        else:
    ...
    response_data = {
        "Credit": "RIZER",
        "Join For More": "Telegram: @Beotherjkman",
        ...
        "login_server_used": login_server_used,
        "update_server_used": result.get("update_server_used")
    }
...
if __name__ == "__main__":
    # Bind to 0.0.0.0 so it's accessible from the network
