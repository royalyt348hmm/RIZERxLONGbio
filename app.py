...
# Login endpoint path
MAJOR_LOGIN_PATH = "/MajorLogin"
# Bio update endpoint path
UPDATE_SOCIAL_PATH = "/UpdateSocialBasicInfo"
...
def perform_major_login(access_token, open_id):
    """
    Try all login base URLs and platform types until one succeeds.
    Returns the JWT token or None.
    """
    platforms = [8, 3, 4, 6]
    # Try each login URL first, then each platform type
    for base_url in LOGIN_BASE_URLS:
        login_url = base_url + MAJOR_LOGIN_PATH
        for platform_type in platforms:
            try:
                ...
                response = requests.post(login_url, data=edata, headers=LOGIN_HEADERS, verify=False, timeout=10)

                if response.status_code == 200:
                    # Attempt to parse token from response
                    try:
                        ...
                        # If parsing fails, try to extract token manually (optional)
                        pass
            except Exception:
...
def upload_bio_request(jwt_token, bio_text):
    """
    Try all server base URLs until one succeeds.
    Returns a dict with status and details.
    """
    last_error = None
    for base_url in SERVER_BASE_URLS:
        update_url = base_url + UPDATE_SOCIAL_PATH
        try:
            data = BioData()
            data.field_2 = 17
            ...
            # If success, return immediately; otherwise store last error and continue
            if resp.status_code == 200:
                return {
                    "status": status_text,
    ...
    if last_error:
        ...
        return {"status": "All servers failed", "code": 500, "bio": bio_text, "server_response": "N/A"}
...
def combined_bio_upload():
    ...
    # Include which server was used if available
    if "server_used" in result:
        response_data["server_used"] = result["server_used"]
